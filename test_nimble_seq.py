...
class TrajectoryTrackingConfig:
    ...
    verify_id_tracking: bool = False
    # Dataset frame range to optimize over
    frame_start: int = 15
    num_frames: int = 72
    object_id: int = 1
    # Optional path to pre-saved controls used as the warm-start for attempt 0.
    # If None or the file doesn't exist, falls back to inverse-dynamics initialisation.
    init_controls_path: Path = None
...
def compute_table_height(object_mesh_file: Path, object_origin_z: float) -> float:
    """Compute the table surface height from the object mesh and its world-frame z at t=0.

    The object rests on the table, so the table top equals the z coordinate of
    the bottom face of the object's bounding box in world space:
        table_top_z = object_origin_z + centroid_z - half_height_z
    """
    import trimesh
    loaded = trimesh.load(str(object_mesh_file), force="mesh")
    cz = float(loaded.bounding_box.centroid[2])
    sz = float(loaded.bounding_box.extents[2])
    return object_origin_z + cz - sz / 2 - 0.05
...
def prepare_scene_assets(
    ...
    return SceneAssets(
        hand_urdf=hand_urdf,
        ground_urdf=create_ground_urdf(temp_dir_path, top_z=table_top_z),
...
def build_grasp_scene(
    ...
    set_dof_positions(obj, ROOT_JOINT_DOF_NAMES, object_pose_t0 + np.array([0, 0, 0, 0, 0, 0]))  # ensure it's a copy, not a view
...
def compute_object_trajectory(
    ...
    Rotation is a rotation vector (exponential map) matching Nimble's FreeJoint convention.
    """
    poses = []
...
def rollout_trajectory_torch(
    ...
    Returns:
        ...
        and "per_step_metrics" (list of dicts with joint_rmse, obj_pos_err, tau_rms
    ...
    for t in range(T - 1):
        ...
        # Per-step metrics (joint_rmse, obj_pos_err, tau_rms)
        if collect_states:
            finger_pos_sim = state[finger_indices_world].detach()
            ...
            obj_pos_err = float(torch.norm(obj_pose_sim[3:] - desired_obj_pose[3:]).item())
...
def optimize_trajectory_control(
    ...
    # Try to load pre-saved controls for the attempt-0 warm-start.
    saved_controls = None
    if config.init_controls_path is not None and Path(config.init_controls_path).exists():
        saved_np = np.load(config.init_controls_path).astype(np.float32)
        if saved_np.shape[1] != finger_dofs:
            print(f"Warning: saved controls have {saved_np.shape[1]} finger DOFs but "
                  f"expected {finger_dofs}. Ignoring.")
        else:
            n_saved = saved_np.shape[0]
            n_needed = T - 1
            if n_saved < n_needed:
                # Pad by repeating the last row.
                pad = np.repeat(saved_np[-1:], n_needed - n_saved, axis=0)
                saved_np = np.vstack([saved_np, pad])
            elif n_saved > n_needed:
                saved_np = saved_np[:n_needed]
            saved_controls = torch.tensor(saved_np, dtype=torch.float32)
            print(f"Loaded init controls from {config.init_controls_path}  "
                  f"(original {n_saved} steps → padded/trimmed to {n_needed})")

    # Compute inverse-dynamics warm-start as fallback for later attempts.
    ...
    try:
        for attempt_idx in range(config.num_attempts):
            if attempt_idx == 0 and saved_controls is not None:
                # Warm-start from pre-saved controls.
                finger_controls = torch.nn.Parameter(saved_controls.clone())
            elif attempt_idx == 0:
                # Fall back to inverse dynamics.
                finger_controls = torch.nn.Parameter(id_controls.clone())
            else:
                # Subsequent attempts add increasing random perturbations around ID controls.
                finger_controls = torch.nn.Parameter(
...
def render_states_to_video(
    ...
    Args:
        ...
        assets: SceneAssets with hand/ground/object URDF paths.
    ...
    def _parse_urdf_visuals(urdf_path: Path, default_color):
        """Return {link_name: [(trimesh.Trimesh, offset_4x4), ...]}."""
    ...
    urdf_color_map = {
        ...
        assets.object_urdf: [0.90, 0.70, 0.20, 1.0],
    }
    all_visuals: dict = {}  # link_name → [(trimesh, offset_4x4, color), ...]
    for urdf_path, color in urdf_color_map.items():
        parsed = _parse_urdf_visuals(urdf_path, color)
        all_visuals.update(parsed)
    ...
    # node_handles: list of (skel_idx, body_idx, offset_4x4, pyrender.Node)
    node_handles = []
...
def _block_until_quit(gui: nimble.NimbleGUI):
    ...
    print("GUI running at http://localhost:8080  —  press Ctrl-C to quit.")
    try:
...
def main():
    ...
    config = TrajectoryTrackingConfig(
        init_controls_path=root_dir / "best_controls3.npy",
        loss_log_path=root_dir / "hoi_tracking_loss_log.csv",
    )
    problem = load_sequence_setup(root_dir, config)

    # desired_object_traj[:, 3:6] = [pos_x, pos_y, pos_z] in ROOT_JOINT_DOF_NAMES order.
    # At t=0 the object is resting on the table; derive the table surface height so
    # the simulation ground matches the real-world table.
    object_z_t0 = float(problem["desired_object_traj"][0, 5])
    table_top_z = compute_table_height(problem["object_mesh_file"], object_z_t0)
    print(f"Table surface height: {table_top_z:.4f}m  (object origin z at t=0: {object_z_t0:.4f}m)")

    with TemporaryDirectory(prefix="nimble_traj_") as temp_dir:
        temp_dir_path = Path(temp_dir)
        assets = prepare_scene_assets(
            temp_dir_path=temp_dir_path,
            hand_urdf=hand_urdf,
            object_mesh_file=problem["object_mesh_file"],
            object_name=problem["object_name"],
            table_top_z=table_top_z,
        )

        # Open the GUI once with its own dedicated world before optimization
        # begins.  This world is never touched by the optimization rollouts, so
        # there is no sharing conflict.  The browser can connect at any time and
        # will see the result once we push states after optimization completes.
        gui_world, _, _ = build_grasp_scene(
            assets=assets,
            config=config,
            hand_finger_joint_names=problem["hand_finger_joint_names"],
            hand_qpos_t0=problem["hand_qpos_sequence"][0],
            object_pose_t0=problem["desired_object_traj"][0],
        )
        gui = nimble.NimbleGUI(gui_world)
        gui.serve(8080)
        add_world_axes(gui)
        print("GUI open at http://localhost:8080 — will update after optimization.")

        if config.verify_id_tracking:
            print("Computing ID controls for tracking verification...")
            id_controls = compute_id_initial_controls(
                assets=assets,
                config=config,
                hand_finger_joint_names=problem["hand_finger_joint_names"],
            ...
            print("Rolling out ID controls to verify hand tracking...")
            # with torch.no_grad():
            #     result = rollout_trajectory_torch(
            #         assets=assets,
            #         config=config,
            #         hand_finger_joint_names=problem["hand_finger_joint_names"],
            #         hand_qpos_seq=problem["hand_qpos_sequence"],
            #         desired_object_traj=problem["desired_object_traj"],
            #         finger_controls=id_controls,
            #         gui=gui
            #     )
            # gui.loopStates([s.detach() for s in result["states"]])
            # _block_until_quit(gui)

            rollout_hand_tracking(
                assets=assets,
                config=config,
                hand_finger_joint_names=problem["hand_finger_joint_names"],
                hand_qpos_seq=problem["hand_qpos_sequence"],
            ...
            input("Inspect hand tracking in browser, then press Enter to start optimization...")

        # best = optimize_trajectory_control(
        #     assets=assets,
        #     config=config,
        #     hand_finger_joint_names=problem["hand_finger_joint_names"],
        ...
        # np.save(root_dir / "best_hoi_controls4.npy", best["controls"].cpu().numpy())

        best_controls = torch.from_numpy(np.load(root_dir / "best_hoi_controls4.npy")).float()
        ...
        if config.render_result:
            ...
            if metrics:
                metrics_path = root_dir / "nimble_seq_metrics.csv"
                with open(metrics_path, "w", newline="", encoding="utf-8") as f:
                    writer = csv.writer(f)
                    writer.writerow(["frame", "joint_rmse", "obj_pos_err", "tau_rms"])
                    for m in metrics:
                        writer.writerow([m["frame"], m["joint_rmse"], m["obj_pos_err"], m["tau_rms"]])
                print(f"Saved metrics -> {metrics_path}")

                joint_rmses = [m["joint_rmse"] for m in metrics]
                obj_errs = [m["obj_pos_err"] for m in metrics]
            ...
            # video_path = root_dir / "result_trajectory4.mp4"
